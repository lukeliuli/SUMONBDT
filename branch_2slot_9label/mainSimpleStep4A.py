...
print("mainSimpleStep4为简化的步骤4程序，注意只能接mainSimpleStep3的输出结果，用于分析幽灵堵车")
...
for j in range(nSamples):
    ...
    if yKerasSumoFlag == 0: 
        ...
        if  numFrontVeh >0 and  meanSpeed>= 15/3.6:
        ...
        if  numFrontVeh >0 and meanSpeed  >= 15/3.6:
            ...
            f2 = dist/speed
            simpleFeatures2.append([f1,f2,f1/f2])
        
        if numFrontVeh >0 and meanSpeed  < 5/3.6:
            f1 = numFrontVeh*2+redTimeLeft
            f2 = dist/speed
            simpleFeatures1.append([f1,f2,f1/f2])
...
df1=pd.DataFrame(simpleFeatures1,columns=['f1','f2','f3'])
df1.to_csv("./data/simpleFeatures1.csv",index=False,header=  ['f1','f2','f3'])


df2=pd.DataFrame(simpleFeatures2,columns=['f1','f2','f3'])
df2.to_csv("./data/simpleFeatures2.csv",index=False,header= ['f1','f2','f3'])


            
            
plt.boxplot([df1['f1'],df2['f1']],showfliers=False,labels=['df1_f1','df2_f1'])
plt.title('f1')
plt.savefig('./data/Step4py_tmp1.png')  # 保存为png格式图片
plt.close()

plt.boxplot([df1['f2'],df2['f2']],showfliers=False,labels=['df1_f2','df2_f2'])
plt.title('f2')
plt.savefig('./data/Step4py_tmp2.png')  # 保存为png格式图片
plt.close()

plt.boxplot([df1['f3'],df2['f3']],showfliers=False,labels=['df1_f3','df2_f3'])
plt.title('f3')
plt.savefig('./data/Step4py_tmp3.png')  # 保存为png格式图片
plt.close()
...
plt.subplot(211) 
plt.hist(x = [df1['f3']],            # 绘图数据
         edgecolor='black',
        bins = 20,            # 指定直方图的条形数为20个
        color = ['r'],    # 指定直方图的填充色
        label = ['df1_f3,< 5/3.6'],     # 为直方图呈现图例
...
plt.subplot(212) 
plt.hist(x = [df2['f3']],            # 绘图数据
         edgecolor='black',
        bins = 20,            # 指定直方图的条形数为20个
        color = ['r'],    # 指定直方图的填充色
        label = ['df2_f3,>= 15/3.6'],     # 为直方图呈现图例
        density = True,      # 是否将纵轴设置为密度，即频率
        alpha = 0.6,          # 透明度
        rwidth = 1)           # 直方图宽度百分比：0-1
...
plt.savefig('./data/Step4py_tmp4.png')  # 保存为png格式图片
plt.close()


sys.stdout=sys.__stdout__ 
print(df1['f3'].describe())
print(df2['f3'].describe())
