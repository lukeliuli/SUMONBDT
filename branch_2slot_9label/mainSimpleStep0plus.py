...
print("程序编号为0plus，用于根据交通拥堵定义，建立的二层模型进行混淆矩阵分析")
########################################################################################################################
...
def convertY2Hieral(y):
    ...
    #根据交通拥堵定义
#https://www.sohu.com/a/406894782_649849
#[0,20],[20,30],[30,35][35
    hierarchy = [4,9]
    labelDict = {"0":["0123","0"],\
                  "1":["0123","1"],\
...
if 1:# 训练多级模型
    ...
    numEpochs =700 #1500/60/60*5 = 2hour
    ...
    for i in range(len(hierarchy)):   
        ...
        #saveName = "../trainedModes/modelSep-9level%d-%dlayer-2slots-gpu1.h5" %(i,numLayers) #保存地址与注意与8层模型不一样
    ...
    fpk=open('sepTrainedsSamplesAll1-2level.pkf','wb+')  
...
fpk=open('sepTrainedsSamplesAll1-2level.pkf','rb') 
...
for i in range(len(hierarchy)):
        ...
        fs = "test_mat2acc%d-2level.csv" %i
        df.to_csv(fs,index= False, header= False)
        
fpk=open('sepTestRVLSamples1-2level.pkf','wb+')         
